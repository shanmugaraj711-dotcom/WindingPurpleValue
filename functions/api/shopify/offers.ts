...
export const onRequestGet: PagesFunction<Env> = async ({ request, env }) => {
  try {
    ...
    const data = await shopifyGraphQL(shop, accessToken, env, `
      query CartLiftOffers {
        shop {
...
export const onRequestPost: PagesFunction<Env> = async ({ request, env }) => {
  try {
    ...
    `;
    const result = await shopifyGraphQL(shop, accessToken, env, mutation, {
      metafields: [{
        ownerId,
        namespace: METAFIELD_NAMESPACE,
        key: METAFIELD_KEY,
        type: "json",
        value: JSON.stringify(body.offers),
      }],
    });
