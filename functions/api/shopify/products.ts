...
export const onRequestGet: PagesFunction<Env> = async ({ request, env }) => {
  try {
    ...
    let claims: Claims;
    try { claims = await verifyIdToken(idToken, env); } catch { claims = decodeDestination(idToken); }
    const shop = new URL(claims.dest!).hostname;
    const accessToken = await getAdminAccessToken(shop, idToken, env);
    const apiVersion = env.SHOPIFY_API_VERSION || "2026-07";
    const response = await fetch(`https://${shop}/admin/api/${apiVersion}/graphql.json`, {
      method: "POST",
      headers: { "Content-Type": "application/json", "X-Shopify-Access-Token": accessToken },
      body: JSON.stringify({ query: `query CartLiftProducts { products(first: 100, reverse: true) { nodes { id title status totalInventory priceRangeV2 { minVariantPrice { amount currencyCode } maxVariantPrice { amount currencyCode } } } pageInfo { hasNextPage } } }` }),
    });
    if (!response.ok) throw new Error(`Shopify Admin API failed (${response.status}).`);
    const payload = await response.json() as { data?: { products?: { nodes?: unknown[]; pageInfo?: { hasNextPage?: boolean } } }; errors?: Array<{ message?: string }> };
    if (payload.errors?.length) throw new Error(payload.errors.map((e) => e.message || "Shopify GraphQL error").join("; "));
    return Response.json({ products: payload.data?.products?.nodes ?? [], hasNextPage: Boolean(payload.data?.products?.pageInfo?.hasNextPage) });
  } catch (error) {
    if (error instanceof Error && error.message === INVALID_ID_TOKEN) return invalidSessionResponse();
    return Response.json({ error: error instanceof Error ? error.message : "Unable to load products." }, { status: 502 });
  }
};
