...
export const onRequestGet: PagesFunction<AnalyticsEnv> = async ({ request, env }) => {
  try {
    ...
    if (!env.CARTLIFT_ANALYTICS_KV) return response(200, { configured: false, events: {}, total: 0 });
    const data = await env.CARTLIFT_ANALYTICS_KV.get(`analytics:${shop}`, "json") as { events?: Record<string, number>; lastEventAt?: number } | null;
    const events = data?.events ?? {};
    const total = Object.values(events).reduce((sum, value) => sum + (Number.isFinite(value) ? value : 0), 0);
    return response(200, { configured: true, events, total, lastEventAt: data?.lastEventAt ?? null });
  } catch (error) {
