...
async function offersApi(method: "GET" | "POST", offers?: Offer[]): Promise<Offer[]> {
  const response = await fetch("/api/shopify/offers", { method, headers: method === "POST" ? { "Content-Type": "application/json" } : undefined, body: method === "POST" ? JSON.stringify({ offers }) : undefined });
...
function CartLiftApp() {
  ...
  useEffect(() => {
    let cancelled = false;
    fetch("/api/shopify/shop").then(async (r) => {
  ...
  return <div className="min-h-screen bg-gray-50 text-gray-900">
    ...
    <main className="mx-auto max-w-6xl px-6 py-8">
      ...
      {section === "overview" && <section className="rounded-2xl border bg-white p-8 shadow-sm"><p className="text-sm font-medium text-gray-500">CARTLIFT</p><h2 className="mt-1 text-3xl font-semibold tracking-tight">Increase cart value with smarter offers.</h2><p className="mt-3 max-w-2xl text-gray-600">A Shopify-native workspace for offers, product signals, and conversion tracking.</p><div className="mt-8 grid gap-4 md:grid-cols-3"><button type="button" onClick={() => setSection("offers")} className="rounded-xl border p-5 text-left hover:bg-gray-50"><p className="font-medium">Offers</p><p className="mt-2 text-sm text-gray-500">{offers.length} saved · {enabled} active</p></button><button type="button" onClick={() => setSection("insights")} className="rounded-xl border p-5 text-left hover:bg-gray-50"><p className="font-medium">Cart insights</p><p className="mt-2 text-sm text-gray-500">{productsLoading ? "Loading store signals…" : `${products.length} products available`}</p></button><button type="button" onClick={() => setSection("performance")} className="rounded-xl border p-5 text-left hover:bg-gray-50"><p className="font-medium">Performance</p><p className="mt-2 text-sm text-gray-500">Tracking foundation ready</p></button></div>{status.connected && <div className="mt-6 rounded-xl border bg-gray-50 p-5 text-sm"><p className="font-medium">Connected store</p><p className="mt-1 text-gray-600">{status.shop?.name || status.shop?.myshopifyDomain}</p>{status.scope && <p className="mt-1 text-gray-500">Granted scope: {status.scope}</p>}</div>}</section>}
      ...
      {section === "insights" && <section className="rounded-2xl border bg-white p-6 shadow-sm"><div className="flex items-center justify-between"><div><h2 className="text-2xl font-semibold">Cart insights</h2><p className="mt-1 text-sm text-gray-500">Store signals available with the current Shopify permission.</p></div><button type="button" onClick={() => void loadProducts()} disabled={productsLoading} className="rounded-lg border bg-white px-3 py-2 text-sm">{productsLoading ? "Refreshing…" : "Refresh"}</button></div>{productsError && <div className="mt-5 rounded-xl border border-amber-200 bg-amber-50 p-4 text-sm text-amber-900">{productsError}</div>}<div className="mt-6 grid gap-4 md:grid-cols-3"><div className="rounded-xl border p-5"><p className="text-sm text-gray-500">Products</p><p className="mt-1 text-2xl font-semibold">{products.length}</p></div><div className="rounded-xl border p-5"><p className="text-sm text-gray-500">Active products</p><p className="mt-1 text-2xl font-semibold">{activeProducts}</p></div><div className="rounded-xl border p-5"><p className="text-sm text-gray-500">Inventory units</p><p className="mt-1 text-2xl font-semibold">{inventory.toLocaleString("en-IN")}</p></div></div><div className="mt-6 overflow-hidden rounded-xl border"><div className="border-b bg-gray-50 px-4 py-3 text-sm font-medium">Product signals</div>{products.slice(0, 10).map((p) => <div key={p.id} className="flex items-center justify-between border-b px-4 py-3 last:border-0"><div><p className="font-medium">{p.title}</p><p className="text-xs text-gray-500">{p.status}</p></div><p className="text-sm text-gray-600">{p.totalInventory ?? 0} units</p></div>)}{!productsLoading && products.length === 0 && <div className="p-6 text-center text-sm text-gray-500">No products returned.</div>}</div><div className="mt-5 rounded-xl border bg-gray-50 p-4 text-sm text-gray-600">True shopper cart behavior requires storefront/cart event collection. We are not fabricating those metrics; this screen currently uses the granted product scope as the live store signal.</div></section>}

      {section === "performance" && <section className="rounded-2xl border bg-white p-6 shadow-sm"><h2 className="text-2xl font-semibold">Performance</h2><p className="mt-1 text-sm text-gray-500">Offer tracking foundation — ready for real conversion events.</p><div className="mt-6 grid gap-4 md:grid-cols-3"><div className="rounded-xl border p-5"><p className="text-sm text-gray-500">Offer rules</p><p className="mt-1 text-2xl font-semibold">{offers.length}</p></div><div className="rounded-xl border p-5"><p className="text-sm text-gray-500">Active rules</p><p className="mt-1 text-2xl font-semibold">{enabled}</p></div><div className="rounded-xl border p-5"><p className="text-sm text-gray-500">Store connection</p><p className="mt-1 text-2xl font-semibold">{status.connected ? "Ready" : "Offline"}</p></div></div><div className="mt-6 rounded-xl border border-dashed p-6"><p className="font-medium">Next measurement layer</p><p className="mt-2 text-sm text-gray-600">The next step is storefront event tracking for offer views, acceptances, cart changes, and attributed conversions. That will turn this workspace into real performance analytics instead of placeholder numbers.</p></div></section>}
    </main>
  </div>;
}
