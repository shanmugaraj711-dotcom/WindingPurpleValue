...
async function shopifyFetch(input: RequestInfo | URL, init: RequestInit = {}): Promise<Response> {
  // Shopify ID tokens are intentionally very short-lived. App Bridge can return
  // a cached token with less than the full lifetime remaining, so a slow/cold
  // backend request can legitimately reach Shopify after the token expires.
  // When the backend marks the response as retryable, fetch a completely new
  // ID token and retry once instead of surfacing invalid_id_token to the user.
...
function CartLiftApp() {
  ...
  return <div className="min-h-screen bg-gray-50 text-gray-900">
    ...
    <main className="mx-auto max-w-6xl px-6 py-8">
      ...
      {section === "offers" && <section className="rounded-2xl border bg-white p-6 shadow-sm"><div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between"><div><h2 className="text-2xl font-semibold">Offers</h2><p className="mt-1 text-sm text-gray-500">Create and manage offer rules stored on this Shopify store.</p></div><button type="button" onClick={create} disabled={!status.connected || saving} className="rounded-lg bg-gray-900 px-4 py-2 text-sm font-medium text-white disabled:opacity-40">Create offer</button></div>{offerError && <div className="mt-5 rounded-xl border border-amber-200 bg-amber-50 p-4 text-sm text-amber-900"><p className="font-medium">Shopify offer storage needs attention</p><p className="mt-1">{offerError}</p><button type="button" onClick={() => void loadOffers()} className="mt-3 rounded-md border border-amber-300 bg-white px-3 py-1.5 font-medium">Retry</button></div>}{formOpen && <form onSubmit={submit} className="mt-6 rounded-xl border bg-gray-50 p-5"><div className="grid gap-4 md:grid-cols-2"><label className="text-sm font-medium">Offer name<input value={name} onChange={(e) => setName(e.target.value)} placeholder="Weekend cart boost" className="mt-1 w-full rounded-lg border bg-white px-3 py-2 font-normal" /></label><label className="text-sm font-medium">Offer type<select value={type} onChange={(e) => setType(e.target.value as OfferType)} className="mt-1 w-full rounded-lg border bg-white px-3 py-2 font-normal"><option value="percentage">Percentage</option><option value="fixed">Fixed amount</option><option value="free_shipping">Free shipping</option></select></label><label className="text-sm font-medium">Value<input value={value} onChange={(e) => setValue(e.target.value)} inputMode="decimal" className="mt-1 w-full rounded-lg border bg-white px-3 py-2 font-normal" /></label><label className="text-sm font-medium">Minimum cart value<input value={minimum} onChange={(e) => setMinimum(e.target.value)} inputMode="decimal" className="mt-1 w-full rounded-lg border bg-white px-3 py-2 font-normal" /></label></div><div className="mt-4 flex gap-2"><button type="submit" disabled={saving} className="rounded-lg bg-gray-900 px-4 py-2 text-sm font-medium text-white disabled:opacity-40">{saving ? "Saving…" : editing ? "Save changes" : "Create offer"}</button><button type="button" onClick={reset} className="rounded-lg border bg-white px-4 py-2 text-sm font-medium">Cancel</button></div></form>}
        <div className="mt-6 space-y-3">{offers.map((offer) => <div key={offer.id} className="flex flex-col gap-3 rounded-xl border p-4 sm:flex-row sm:items-center sm:justify-between"><div><p className="font-medium">{offer.name}</p><p className="mt-1 text-sm text-gray-500">{offerText(offer)} · minimum ₹{offer.minCartValue.toLocaleString("en-IN")} · {offer.enabled ? "Active" : "Paused"}</p></div><div className="flex gap-2"><button type="button" onClick={() => edit(offer)} className="rounded-md border px-3 py-1.5 text-sm">Edit</button><button type="button" onClick={() => void persist(offers.map((o) => o.id === offer.id ? { ...o, enabled: !o.enabled } : o))} disabled={saving} className="rounded-md border px-3 py-1.5 text-sm">{offer.enabled ? "Pause" : "Enable"}</button></div></div>)}</div></section>}

      {section === "insights" && <section className="rounded-2xl border bg-white p-6 shadow-sm"><h2 className="text-2xl font-semibold">Cart insights</h2><p className="mt-1 text-sm text-gray-500">Real shopper events captured through the CartLift pixel.</p>{analyticsError && <div className="mt-5 rounded-xl border border-amber-200 bg-amber-50 p-4 text-sm text-amber-900">{analyticsError}</div>}<div className="mt-6 grid gap-4 sm:grid-cols-2 lg:grid-cols-4">{["page_viewed", "product_viewed", "product_added_to_cart", "checkout_started"].map((name) => <div key={name} className="rounded-xl border p-5"><p className="text-sm text-gray-500">{name.replaceAll("_", " ")}</p><p className="mt-2 text-3xl font-semibold">{event(name)}</p></div>)}</div><div className="mt-4 rounded-xl border bg-gray-50 p-5"><p className="text-sm text-gray-500">Total shopper events</p><p className="mt-1 text-3xl font-semibold">{analytics.total}</p>{analytics.lastEventAt && <p className="mt-1 text-xs text-gray-500">Last event {new Date(analytics.lastEventAt).toLocaleString()}</p>}</div></section>}

      {section === "performance" && <section className="rounded-2xl border bg-white p-6 shadow-sm"><h2 className="text-2xl font-semibold">Performance</h2><p className="mt-1 text-sm text-gray-500">Signals that help measure whether CartLift improves conversion.</p><div className="mt-6 grid gap-4 md:grid-cols-3"><div className="rounded-xl border p-5"><p className="text-sm text-gray-500">Active products</p><p className="mt-2 text-3xl font-semibold">{activeProducts}</p></div><div className="rounded-xl border p-5"><p className="text-sm text-gray-500">Inventory units</p><p className="mt-2 text-3xl font-semibold">{inventory}</p></div><div className="rounded-xl border p-5"><p className="text-sm text-gray-500">Completed checkouts</p><p className="mt-2 text-3xl font-semibold">{event("checkout_completed")}</p></div></div>{productsError && <div className="mt-5 rounded-xl border border-amber-200 bg-amber-50 p-4 text-sm text-amber-900">{productsError}</div>}</section>}
    </main>
  </div>;
}
